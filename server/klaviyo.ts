...
export async function upsertKlaviyoProfile(
  ...
  const result = await klaviyoRequest<{ data?: { id?: string } }>("POST", "/api/profile-import", {
    data: {
      type: "profile",
  ...
  return { ok: result.ok, profileId, error: result.error };
}
...
export async function syncContactToKlaviyo(
  lead: ContactKlaviyoLead,
): Promise<{ profileId?: string }> {
  if (!isKlaviyoEnabled()) return {};
  const { firstName, lastName } = splitPersonName(lead.name);
  const imported = await upsertKlaviyoProfile({
    email: lead.email,
    phone: lead.phone,
    firstName,
    lastName,
  ...
  return { profileId: imported.profileId };
}
...
export async function syncStartedCheckout(input: {
  ...
  if (!isKlaviyoEnabled()) return {};
  const imported = await upsertKlaviyoProfile({ email: input.email });
...
export async function syncPlacedOrder(order: StoredOrder): Promise<{ profileId?: string }> {
  if (!isKlaviyoEnabled() || !order.customerEmail) return {};
  const imported = await upsertKlaviyoProfile({ email: order.customerEmail });
  const items = parseCheckoutItems(order.items);
