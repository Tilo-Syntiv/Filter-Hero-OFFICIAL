...
Admin and CRM **read** `klaviyo_profile_id`. They must not send mail or write Klaviyo (FH-213). `server/klaviyo.ts` returns the profile id from `POST /api/profile-import`. Contact, checkout, paid orders, and identify stamp it with `attachKlaviyoProfileId` (FH-387). That helper does not create a contact.
