  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <AdminPanel title="Klaviyo">
        {klaviyo ? (
          <div className="space-y-2">
            ...
            {klaviyo.error ? <p className="text-sm text-destructive">{klaviyo.error}</p> : null}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">Checking Klaviyo…</p>
        )}
        <p className="mt-4 text-sm text-muted-foreground">
          Flows, segments, and campaign analytics stay in Klaviyo. This console does not send
          marketing mail.
        </p>
        <a
      ...
      <AdminPanel title="Server events">
        ...
        <p className="mt-3 text-xs text-muted-foreground">Catalog feed: {tracking.data.catalogFeed}</p>
      </AdminPanel>
    </div>
  );
}
